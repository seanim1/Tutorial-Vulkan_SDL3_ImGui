...
struct UniformData {
    float color[3];
    float _pad; // vec3 needs 16-byte alignment in GLSL
};
// ---------------
...
struct AppState {
    ...
    bool show_gui = true;
    float clear_r = 1.0f, clear_g = 0.0f, clear_b = 0.0f;
    // -------------
};
...
{
    UniformData ubo = {
        .color = {app->clear_r, app->clear_g, app->clear_b},
    };
    SDL_Log("uniform color: r=%.3f g=%.3f b=%.3f", ubo.color[0], ubo.color[1], ubo.color[2]);
...
void update_gui(AppState* app)
{
    ...
    if (app->show_gui) {
        ...
        ImGui::Separator();
        ImGui::ColorEdit3("Triangle color", &app->clear_r);
...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[]) {
    ...
    *appstate = app.release();
    ...
    app->window = SDL_CreateWindow("Part 07 - Uniform Buffer", WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_VULKAN);
    if (!app->window) {
