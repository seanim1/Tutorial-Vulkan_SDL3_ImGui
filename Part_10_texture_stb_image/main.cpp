...
void create_uniform_buffers(AppState* app)
{
    ...
    for (uint32_t i = 0; i < app->image_count; i++) {
        ...
        ENGINE_xzlog("vkCreateBuffer (uniform)");
        check_vk(vkCreateBuffer(app->device, &buf_info, nullptr, &app->uniform_buffers[i]), "vkCreateBuffer uniform");

        VkMemoryRequirements mem_reqs;
        vkGetBufferMemoryRequirements(app->device, app->uniform_buffers[i], &mem_reqs);
        ...
        ENGINE_xzlog("vkAllocateMemory (uniform)");
        check_vk(vkAllocateMemory(app->device, &alloc_info, nullptr, &app->uniform_buffers_memory[i]), "vkAllocateMemory uniform");
        check_vk(vkBindBufferMemory(app->device, app->uniform_buffers[i], app->uniform_buffers_memory[i], 0), "vkBindBufferMemory uniform");

        // keep mapped permanently — no need to map/unmap every frame
        vkMapMemory(app->device, app->uniform_buffers_memory[i], 0, sizeof(UniformData), 0, &app->uniform_buffers_mapped[i]);
    }
}
...
void create_texture(AppState* app)
{
    ...
    check_vk(vkCreateBuffer(app->device, &buf_info, nullptr, &staging_buffer), "staging buf");

    VkMemoryRequirements mem_reqs;
    vkGetBufferMemoryRequirements(app->device, staging_buffer, &mem_reqs);
    VkMemoryAllocateInfo alloc_info = {
        .sType           = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
        .allocationSize  = mem_reqs.size,
        .memoryTypeIndex = find_memory_type(app->physical_device, mem_reqs.memoryTypeBits,
    ...
    check_vk(vkAllocateMemory(app->device, &alloc_info, nullptr, &staging_memory), "staging mem");
    vkBindBufferMemory(app->device, staging_buffer, staging_memory, 0);
...
void create_descriptors(AppState* app)
{
    ...
    ENGINE_xzlog("vkCreateDescriptorSetLayout");
    check_vk(vkCreateDescriptorSetLayout(app->device, &layout_info, nullptr, &app->descriptor_set_layout), "vkCreateDescriptorSetLayout");
    ...
    ENGINE_xzlog("vkCreateDescriptorPool");
    check_vk(vkCreateDescriptorPool(app->device, &pool_info, nullptr, &app->descriptor_pool), "vkCreateDescriptorPool");
    ...
    ENGINE_xzlog("vkAllocateDescriptorSets");
    check_vk(vkAllocateDescriptorSets(app->device, &alloc_info, app->descriptor_sets.data()), "vkAllocateDescriptorSets");
...
void update_uniform_buffer(AppState* app, uint32_t image_index)
{
    ...
    app->rotation_y += dt * 4.0f; // auto-rotate Y at 20 degrees per second
...
void init_vulkan(AppState* app) {
    ...
    /* --- Command Pool (needed before create_texture) --- */
    ...
    /* --- Texture (must happen before create_descriptors) --- */
    create_texture(app);

    /* --- Uniform + Descriptor setup (must happen before pipeline layout) --- */
...
void update_gui(AppState* app)
{
    ...
    if (app->show_gui) {
        ImGui::Begin("Textured Tetrahedron", nullptr, ImGuiWindowFlags_AlwaysAutoResize);
...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[]) {
    ...
    *appstate = app.release();
    ...
    app->window = SDL_CreateWindow("Part 10 - Texture", WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_VULKAN);
    if (!app->window) {
